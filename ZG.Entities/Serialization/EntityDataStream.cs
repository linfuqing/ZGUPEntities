...
namespace ZG
{
    ...
    {
        void Deserialize(ref UnsafeBlock.Reader reader, Entity entity, EntityManager entityManager);
    }
    ...
    public struct EntityBufferDeserializer<T> : IEntityDataStreamDeserializer where T : unmanaged, IBufferElementData
    {
        public void Deserialize(ref UnsafeBlock.Reader reader, Entity entity, EntityManager entityManager)
        {
            ...
            if (entityManager.HasComponent<T>(entity))
                ...
                buffer = entityManager.AddBuffer<T>(entity);

            buffer.CopyFrom(reader.ReadArray<T>(reader.Read<int>()));
        }
    }
    ...
    public static class EntityDataStreamUtility
    {
        ...
        public static void DeserializeStream(this in UnsafeBlock block, in Entity entity, EntityManager entityManager)
        {
            ...
            while (reader.isVail)
            {
                ...
                {
                    foreach(var attribute in attributes)
                        ((IEntityDataStreamDeserializer)Activator.CreateInstance(attribute.deserializerType)).Deserialize(ref reader, entity, entityManager);
                }
            }
        }
    }
